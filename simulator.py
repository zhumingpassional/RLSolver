...
from graph_utils import load_graph, GraphList
from graph_utils import build_adjacency_matrix, build_adjacency_indies
from graph_utils import obtain_num_nodes
...
class SimulatorGraphMaxCut:
    def __init__(self, sim_name: str = 'max_cut', graph_list: GraphList = (),
        ...
        '''load graph'''
        graph_list: GraphList = graph_list if graph_list else load_graph(graph_name=sim_name)
    ...
    def calculate_obj_values_for_loop(self, solutions: TEN, if_sum: bool = True) -> TEN:  # 代码简洁，但是计算效率低
        num_sims, num_nodes = solutions.shape
        values = th.zeros((num_sims, num_nodes), dtype=self.int_type, device=self.device)
        for node0 in range(num_nodes):
            node1s = self.adjacency_indies[node0]
            if node1s.shape[0] > 0:
                values[:, node0] = (solutions[:, node0, None] ^ solutions[:, node1s]).sum(dim=1)
    ...
    def generate_solutions_randomly(self, num_sims):
        soluctions = th.randint(0, 2, size=(num_sims, self.num_nodes), dtype=th.bool, device=self.device)
        soluctions[:, 0] = 0
        return soluctions

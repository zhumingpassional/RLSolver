...
"""以下代码即将取代上方的 class MaxcutSimulatorReinforce
Maxcut 这个命名在PyCharm里面提示拼写错误，改成 SimulatorGraphMaxCut

由于当前的repo名称为 Maxcut，所以SimulatorGraphMaxCut 可以改成更加简洁的 Simulator

放在util.py 里的函数，应当是同级目录下，超过1个py文件需要使用的函数。
...
全局变量：GraphList，IndexList，DataDir
不应该放在simulator.py 里，下一次 PR 需要把它们放到合适位置
"""

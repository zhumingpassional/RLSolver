...
class iSCO:
    ...
    def step(self, x, path_length, temperature):
        cur_x = x.clone()
        log_acc = torch.zeros((BATCH_SIZE),dtype = torch.float,device=self.device)
        traj = torch.zeros((BATCH_SIZE,3,path_length),dtype=torch.float,device=self.device)
        # distance_x = self.calculate_distance(cur_x)
        for i in range(path_length):
            cur_x, logits, trajectory,delta_yx = self.proposal(cur_x, temperature)
            #这两条是调试用的，看看有没有换错，跑的时候在报错的地方打个断点。
            # distance_y = self.calculate_distance(cur_x)
            # a = distance_y-distance_x+delta_yx
        ...
        # log_acc = torch.clamp(torch.sum(log_acc), max=0.0)
        #torch.sum(traj[:,0,:],dim=1)
    ...
    def proposal(self, sample, temperature):
        x = sample.clone()
        #这一部分写的很乱，switch可能有错
        #logits:delat(fx)/2,log_prob:log(每个点的采样权重)，indices：sample中的每个点要交换的点在sample中的索引，：ban_mask：不进行交换的点，True为不交换
    ...
    def random_gen_init_sample(self, params_dict):
        ...
        # sample = torch.randperm(self.num_nodes, dtype=torch.long, device=self.device)
        return sample

...
class SpinSystemBase(ABC):
    ...
    class action_space():
        def __init__(self, n_actions):
            self.n = n_actions
            self.actions = torch.arange(self.n,device=DEVICE)

        def sample(self, n=1):
            return torch.tensor(self.actions,device=DEVICE)[torch.multinomial(torch.ones(len(self.actions)), n, replacement=True)].tolist()

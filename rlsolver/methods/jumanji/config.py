import torch as th
from torch.cuda import graph
from rlsolver.methods.config import GraphType
import os
cur_path = os.path.dirname(os.path.abspath(__file__))
rlsolver_path = os.path.join(cur_path, '../../')

GRAPH_TYPE = GraphType.BA

GPU_ID = 0

NUM_TRAIN_NODES = 200
NUM_TRAIN_SIMS = 2 ** 4
NUM_VALIDATION_NODES = 400
HERIZON_LENGTH = int(NUM_TRAIN_NODES/2)
VALIDATION_SEED = 10
NUM_VALIDATION_SIMS = 2 ** 4
TEST_SAMPLING_SPEED = True

#推理的参数
INFERENCE_GPU_ID = 7
NUM_INFERENCE_SIMS = 50
MINI_INFERENCE_SIMS = 50 
NUM_INFERENCE_NODES = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 2000, 3000, 4000, 5000,10000]
USE_TENSOR_CORE = True 
INFERENCE_PREFIXES = [GRAPH_TYPE.value + "_" + str(i) + "_" for i in NUM_INFERENCE_NODES]
NUM_TRAINED_NODES_IN_INFERENCE = 20

NEURAL_NETWORK_SAVE_PATH = rlsolver_path + "methods/jumanji/pretrained_agent/tmp/_BA_200_16_f/network_best.pth"
DATA_DIR = rlsolver_path + "/data/syn_" + GRAPH_TYPE.value
RESULT_DIR = rlsolver_path + "/methods/jumanji/pretrained_agent/tmp"
NETWORK_FOLDER = rlsolver_path + "/methods/jumanji/pretrained_agent/tmp/"+""

SAVE_NETWORK_FREQUENCY = 10
TEST_FREQUENCY = 100






def calc_device(gpu_id: int):
    return th.device(f'cuda:{gpu_id}' if th.cuda.is_available() and gpu_id >= 0 else 'cpu')

TRAIN_DEVICE = calc_device(GPU_ID)
INFERENCE_DEVICE = calc_device(INFERENCE_GPU_ID)

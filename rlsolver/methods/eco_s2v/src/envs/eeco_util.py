...
#         """
#         self.env_shape = n_sims
#         N1 = self.env_shape

#         # buffers[i][j] = { hist_len: set_of_frozensets }
#         self.buffers = [dict()  for _ in range(N1)]

#         # 当前每个环境的动作集合与长度
#         self.current_action_hists = [set()  for _ in range(N1)]
#         self.current_action_hist_lens = [0 for _ in range(N1)]
...
class HistoryBuffer():
    ...
    def update(self, actions):
        """
        actions: 一个长度为 n 的 tensor，每个元素表示对应环境的 action
        """
        ...
        for env_idx, action in enumerate(actions):  # 遍历每个环境
            ...
            try:
                list_of_states = self.buffer[self.current_action_hist_len[env_idx]]
                if new_action_hist in list_of_states:
            ...
            list_of_states.append(new_action_hist)
            self.current_action_hist[env_idx] = new_action_hist
            self.buffer[self.current_action_hist_len[env_idx]] = list_of_states

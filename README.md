...
Pattern II: policy-based methods first formulate the CO problem as a QUBO problem, and then learn a policy using say REINFORCE algorithm to minimize the Hamiltonian objective function. Here, the policy $\pi$, is a probability vector denoting the node is belong to the set. For example, the policy for a graph with 3 nodes is [0, 0, 0.9] means the probabilities of the first two nodes belong to the set are 0, and the probability of the third node belong to the set is 0.9. 

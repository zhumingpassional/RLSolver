...
1. reset(): Generate a random initial solution. 
2. step(): Search for better solutions based on the current solution. It has two sub-functions. 
...
3. pick\_good\_xs(): Select the good solutions in all parallel environments, where each environment returns exactly one good solution with corresponding objective value.
4. obj(): Calculate the objective value.

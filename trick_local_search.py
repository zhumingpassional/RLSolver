...
class TrickLocalSearch:
    def __init__(self, simulator: GraphMaxCutSimulator, num_nodes: int):
...
def check_generate_best_x():
    ...
    '''simulator'''
    sim = GraphMaxCutSimulator(sim_name=sim_name, device=device)
...
hUH4vj0nAzi24"""
    ...
    simulator_class = GraphMaxCutSimulator
    trick_class = TrickLocalSearch
    ...
    '''trick'''
    trick = trick_class(simulator=sim, num_nodes=sim.num_nodes)
    ...
    for j2 in range(num_reset):
        ...
        for j1 in range(_num_iter1):
            ...
            update_xs_by_vs(best_xs, best_vs, trick.good_xs, trick.good_vs)

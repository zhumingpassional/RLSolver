...
def hamiltonian(samples, qubo, offset):
    """
...
class vca:
    ...
    def __init__(self, N, n_layers, n_warmup, n_anneal, n_train, qubo, offset, RNNtype, rnn_unit, T0):
        """
        ...
        qubo - qubo matrix of an NSP instance used with an offset to compute the energy of solutions to the NSP
        offset - real constant of an NSP instance used with an offset to compute the energy of solutions to the NSP
    ...
    def run(self):
        """
        vca.run() builds an RNN architecture based on the choice of hyperparameters provided in vca.__init__(). The model then autoregressively generates numsamples (see line 450) 
        ...
        with tf.compat.v1.variable_scope(self.PRNN.scope,reuse=tf.compat.v1.AUTO_REUSE):
            with self.PRNN.graph.as_default():
                ...
                for it in range(self.num_warmup):
                    ...
                    energies = hamiltonian(samples, self.qubo, self.offset)
                ...
                for i in range(n_steps):
                    ...
                    energies_step = hamiltonian(samples_step, self.qubo, self.offset)
